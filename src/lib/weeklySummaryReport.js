...
// runs), no separate backend endpoint. Headline number first, supporting
// detail below, deliberately not a raw data dump: this is meant to be
// forwardable to a client's own leadership, not read by a QA engineer.
export function generateWeeklySummaryPdf({ project, data, bugs = [], runs = [], automationRuns = [] }) {
  const now = new Date()
